...
describe("move 規則", () => {
  ...
  test("移動到有玩家佔據嘅格會失敗（避免疊格）", () => {
    ...
    // P1 喺 (1,1)，P2 喺 (2,1)，P1 嘗試移動到 (2,1)（P2 佔據）
    p1.position = { x: 1, y: 1 };
    p2.position = { x: 2, y: 1 };
...
describe("advantage 規則", () => {
  test("拳打武器時有 advantage，傷害會增加", () => {
    ...
    p1.position = { x: 1, y: 1 };
    p2.position = { x: 1, y: 2 };
    // 固定朝向，令測試專注於 advantage 規則（p1 面向 up，p2 喺下方為背面）
    p1.facing = "up";
...
describe("target invalidation / retargeting", () => {
  test("attack 帶 retargetInstruction.toTargetId 時，會改為命中新目標", () => {
  const state = createMatch();
  const p1 = state.players[0];
  const p2 = state.players[1];
...
describe("target validity contract", () => {
  test("當唯一目標不合法時，attack 不會造成傷害並會記錄失敗原因", () => {
  const state = createMatch();
  const [p1, p2] = state.players;
  ...
  expect(
    state.log.some(
      (msg) => msg.includes("沒有合法目標") || msg.includes("目標已失效")
    )
  ).toBe(true);
});
});
