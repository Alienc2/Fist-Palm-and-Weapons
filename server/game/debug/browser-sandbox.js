...
function renderEmptyState() {
  ...
  refs.logOutput.innerHTML = '<p class="muted-text">No log yet.</p>';
  setStatus("idle", "Idle");
}
...
function runScenarioStub(scenario) {
  ...
  if (scenario.name === "move-vs-defense") {
    ...
    log.push("P2 used defense basic_guard_2 and keeps the effect until triggered or round end");
  }
  ...
  if (scenario.name === "attack-vs-attack") {
    ...
    log.push("P2 used basic_palm_1 targeting P1, but range was invalid");
    log.push("P1 used basic_punch_1 targeting P2, but range was invalid");
  }
  ...
  if (scenario.name === "buy-vs-idle") {
    ...
    log.push("P1 bought shop_mp_1 successfully, spent 2 MP, remaining stock 0");
  }
...
async function handleRun() {
  ...
  try {
    ...
    refs.adapterModeText.textContent = "Adapter mode: adapter-error";
